...
configure_instance = [
  ...
  {"action":"apt",
    "params":["libpq-dev", "nginx", "memcached", "git",
      "python-setuptools", "python-dev", "libmysqlclient-dev", "build-essential", "python-pip"],
  ...
  # EDIT stijn: not needed that's part of our sheldonize requirements file
  #{"action":"virtualenv", "params":"pip install Django",
  #  "message":"Installing django"},

  # install psycopg2 drivers for Postgres
  # EDIT: Stijn we do not need those drivers (we'll be using MySQL see the
  # libmysql installation above)
  ...
  {"action":"put", "params":{"file":"%(SSH_PATH)s/www.sheldonize.com.crt",
                            "destination":"/home/%(SERVER_USERNAME)s/www.sheldonize.com.crt"}},
  {"action":"put", "params":{"file":"%(SSH_PATH)s/www.sheldonize.com.key",
                            "destination":"/home/%(SERVER_USERNAME)s/www.sheldonize.com.key"}},
  {"action":"sudo", "params":"mv /home/%(SERVER_USERNAME)s/www.sheldonize.com.crt /etc/nginx/www.sheldonize.com.crt"},
  {"action":"sudo", "params":"mv /home/%(SERVER_USERNAME)s/www.sheldonize.com.key /etc/nginx/www.sheldonize.com.key"},
  {"action":"sudo", "params":"chown root:root /etc/nginx/www.sheldonize.com.crt"},
  {"action":"sudo", "params":"chown root:root /etc/nginx/www.sheldonize.com.key"},
  {"action":"run", "params":"chmod 600 /home/%(SERVER_USERNAME)s/.ssh/%(BITBUCKET_DEPLOY_KEY_NAME)s"},
