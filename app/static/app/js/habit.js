function markdownlogic(id) {
    ...
    // The comment editing
$("#habit-comment").markdown({autofocus:false,savable:true,
        onShow: function(e){
    ...
    $("#habit-edit").click(function(){
        ...
        $('#habit-edit').remove();
        $("#habit-comment").markdown({autofocus:false,savable:true,height:"400",

            onSave: function(e) {
                // Send note to background
                $('#save-status').html('Saving...');
                $.ajax({
                    url:"/app/habits/note/ajax/",
                    type:"POST",
...
$( document ).ready(function() {
    ...
    var id = $("#habit-comment").attr( "habit-id" )
        markdownlogic(id);
